...
rccf<-ddply(yb, .(province), function(x) {
  ...
  x$rateCumSum<-cumsum(x$rate)
  x<-removeSeasonalDF(standardizationDF(x, id.vars=c("province", "year", "month")), 10, id.vars=c("province", "year", "month"), rt=c("t"))
#   calmcor(x[,c("xiangmushu", "zhufangmianji", "zhufangtouzi")], x[, c("rate", "rateCumSum")], paste("../../data/sanyIndex/result/", prov, "cor.csv", sep=""))
  calCCF(x[,c("xiangmushu", "zhufangmianji", "zhufangtouzi")], x[, c("rate", "rateCumSum")], paste("../../data/sanyIndex/result/", prov, "ccf.csv", sep=""), FALSE)
})
